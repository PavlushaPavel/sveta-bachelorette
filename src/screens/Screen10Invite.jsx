...
export default function Screen10Invite({ onNext }) {
  return (
    <div className="screen">
      <div style={{ textAlign: 'center', animation: 'fadeUp 400ms ease-out 100ms both' }}><p style={{ fontSize: '2.5rem' }}>🥂</p></div>
      ...
      <div className="screen-body">
        ...
        <div style={{ marginTop: 'var(--space-sm)', paddingLeft: 'var(--space-md)', color: 'var(--color-muted)' }}>
          {LIST.map((item, i) => <p key={i} style={{ animation: `fadeUp 180ms ease-out ${400 + i * 60}ms both` }}>— {item}</p>)}
        </div>
      </div>
